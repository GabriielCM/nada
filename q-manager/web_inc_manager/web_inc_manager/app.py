...
def main_menu():
    ...
    # INCs recentes (últimas 5)
    incs_recentes = INC.query.order_by(INC.id.desc()).limit(5).all()
    
    # Dados para o gráfico de INCs por mês
    monthly_data = []
    
    # Coletar dados dos últimos 6 meses
    for i in range(5, -1, -1):
        current_date = datetime.today() - timedelta(days=30 * i)
        month_start = current_date.replace(day=1).strftime('%d-%m-%Y')
        month_end = (current_date.replace(day=28) + timedelta(days=4)).replace(day=1)
        month_end = (month_end - timedelta(days=1)).strftime('%d-%m-%Y')
        
        # Contar INCs no mês
        count = INC.query.filter(INC.data >= month_start, INC.data <= month_end).count()
    ...
    # Dados para o gráfico de distribuição por urgência
    critico = INC.query.filter_by(urgencia='Crítico').count()
    moderada = INC.query.filter_by(urgencia='Moderada').count()
    leve = INC.query.filter_by(urgencia='Leve').count()
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if registros:
                ...
                flash('Nenhum registro válido foi importado.', 'warning')
                return redirect(request.url)
